...
export class LeaderboardRedisRepository implements LeaderboardRepository {
  async getCachedLeaderboard(banlistname: string): Promise<Leaderboard | null> {
    ...
    const response = await client.get(this.buildFilter(banlistname));
    if (!response) {
      return null;
    }
    ...
    return new Leaderboard(
      (data.leaderboard as LeaderboardItemRedis[]).map(
        (item) =>
          new LeaderboardItem({
            ...item,
            rank: item.position,
  ...
  async get(banlistname: string): Promise<Leaderboard> {
    if (banlistname === "general") {
      const client = await RedisClientFactory.createClient();
      const points = await client.ZRANGEBYSCORE_WITHSCORES(
        `leaderboard:points`,
        "-inf",
        "+inf"
      );
      const wins = await client.ZRANGEBYSCORE_WITHSCORES(
        `leaderboard:wins`,
        "-inf",
        "+inf"
      );
      const losses = await client.ZRANGEBYSCORE_WITHSCORES(
        `leaderboard:losses`,
        "-inf",
        "+inf"
      );
      await client.quit();
      const leaderboardItems = await this.buildLeaderboardItems(
        points,
        wins,
        losses
      );
    ...
    const client = await RedisClientFactory.createClient();
    const points = await client.ZRANGEBYSCORE_WITHSCORES(
      `leaderboard:${banlistname}:points`,
      "-inf",
      "+inf"
    );
    const wins = await client.ZRANGEBYSCORE_WITHSCORES(
      `leaderboard:${banlistname}:wins`,
      "-inf",
      "+inf"
    );
    const losses = await client.ZRANGEBYSCORE_WITHSCORES(
      `leaderboard:${banlistname}:losses`,
      "-inf",
      "+inf"
    );
    await client.quit();
  ...
  async cache(leaderboard: Leaderboard, banlistname: string): Promise<void> {
    const client = await RedisClientFactory.createClient();
    await client.set(
			this.buildFilter(banlistname),
      JSON.stringify(leaderboard.toPresentation())
    );
    await client.quit();
  }

  private async buildLeaderboardItems(
    points: RedisRanking[],
    wins: RedisRanking[],
    losses: RedisRanking[]
  ...
  private buildFilter(banlistname: string): string {
    if (banlistname === "general") {
      return "ranking";
    }

    return `ranking:${banlistname}`;
  }
}
